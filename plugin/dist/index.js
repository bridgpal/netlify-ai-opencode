...
async function fetchModels(url, key) {
    ...
    if (!res.ok)
        throw new Error(`relay /models returned ${res.status}`);
    const json = (await res.json());
    if (!json.data?.length)
        throw new Error("relay /models returned no models");
    return json.data;
}
function toModel(url, m) {
...
const plugin = {
    ...
    setup: async (ctx) => {
        ...
        const upstreams = new Set(opts.upstreams ?? ["anthropic", "openai", "gemini", "openrouter"]);
